...
const cvData = {
  ...
  summary: [
    "Highly accomplished culinary professional bringing over two decades of international kitchen leadership, including Executive Chef, Head Chef and Sous Chef experience. Applying proven expertise in high volume production managing 250 350 daily covers, menu development, and consistently achieving 95%+ guest satisfaction and 99%+ quality standards across diverse dining operations.",
    "Expertise includes the ability to lead, train, and supervise junior staff, manage mise en place, and confidently oversee all sections for all day dining and Room Service. Holds a current Food Safety Certificate (NCEA Level 3) and is committed to upholding the highest food hygiene standards."
  ],
  ...
  <header className="bg-blue-600 text-white px-8 py-6 rounded-t-lg shadow-xl border-b-4 border-blue-400 print:shadow-none print:px-0 print:py-6 print:bg-white print:text-slate-900 print:border-blue-600">
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center print:flex-row print:items-center">
      <div>
        ...
        <p className="text-xl font-medium text-blue-200 mt-1 uppercase tracking-wider print:text-blue-700 print:text-lg">{personal.title}</p>
      </div>
...
const ContactItem = ({ icon, text, link }) => (
  <div className="flex items-center space-x-2 justify-start md:justify-end">
    <Icon path={icon} className="w-4 h-4 text-blue-200 print:text-blue-600 flex-shrink-0" />
    {link ? (
      <a href={link} target="_blank" rel="noopener noreferrer" className="hover:text-blue-200 transition duration-150 border-b border-transparent hover:border-blue-200 print:hover:text-blue-600">
        {text}
      </a>
    ) : (
      <span>{text}</span>
...
const ExperienceItem = ({ role, company, location, dates, description }) => (
  <div className="experience-item mb-5 p-5 rounded border break-inside-avoid print:mb-4 print:p-0 print:border-none bg-white border-blue-100 shadow-sm">
    ...
    <div className="min-h-screen bg-slate-100 p-4 sm:p-12 font-sans text-slate-900 print:bg-white print:p-0">
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-2xl overflow-hidden border border-slate-200 print:shadow-none print:border-none print:max-w-none print:w-full">
        ...
        <main className="p-8 md:p-12 print:p-0 print:pt-4">
          ...
          {/* Skills & Strengths */}
          <section className="break-inside-avoid">
            <SectionTitle title="Culinary & Management Skills" icon="briefcase" />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 print:gap-6 print:grid-cols-2">
              <div>
                <h3 className="text-lg font-bold text-slate-800 mb-3 uppercase tracking-tight print:text-sm">Culinary Expertise</h3>
                <div className="flex flex-wrap gap-2">
                  {skills.culinary.map((skill, i) => <SkillBadge key={i} text={skill} />)}
                </div>
              </div>

              <div>
                <h3 className="text-lg font-bold text-slate-800 mb-3 uppercase tracking-tight print:text-sm">Kitchen Operations</h3>
                <div className="flex flex-wrap gap-2">
                  {skills.operations.map((skill, i) => <SkillBadge key={i} text={skill} />)}
                </div>
              </div>

              <div>
                <h3 className="text-lg font-bold text-slate-800 mb-3 uppercase tracking-tight print:text-sm">Certifications</h3>
                <div className="flex flex-wrap gap-2">
                  {skills.certifications.map((skill, i) => <SkillBadge key={i} text={skill} />)}
                </div>
              </div>

              <div>
                <h3 className="text-lg font-bold text-slate-800 mb-3 uppercase tracking-tight print:text-sm">Leadership & Management</h3>
                <div className="flex flex-wrap gap-2">
                  {skills.leadership.map((skill, i) => <SkillBadge key={i} text={skill} />)}
                </div>
              </div>

              <div className="md:col-span-2">
                <h3 className="text-lg font-bold text-slate-800 mb-3 uppercase tracking-tight print:text-sm">Communication</h3>
                <div className="flex flex-wrap gap-2">
                  {skills.communication.map((skill, i) => <SkillBadge key={i} text={skill} />)}
                </div>
              </div>
            </div>
          </section>
